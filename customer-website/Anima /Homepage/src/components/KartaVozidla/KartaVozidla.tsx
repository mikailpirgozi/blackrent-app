...
export const KartaVozidla = ({
  ...
  return (
    <div
      className={`w-[328px] flex flex-col items-start gap-[23px] p-4 h-[392px] overflow-hidden rounded-3xl justify-around relative ${type === "hover" ? "bg-colors-black-600" : "bg-colors-black-400"} ${className}`}
    >
      <div
        className={`w-full flex self-stretch flex-col items-start grow gap-6 flex-1 relative ${frameClassName}`}
      >
        <div
          className={`w-full self-stretch bg-cover h-56 rounded-lg bg-[50%_50%] relative ${["tag-DPH", "tag-discount-DPH"].includes(type) ? "flex" : ""} ${["tag-DPH", "tag-discount-DPH"].includes(type) ? "items-start" : ""} ${["tag-DPH", "tag-discount-DPH"].includes(type) ? "gap-1" : ""} ${["tag-DPH", "tag-discount-DPH"].includes(type) ? "p-2" : ""} ${type === "default" ? "bg-[url(/img/n-h-ad-vozidla.png)]" : (type === "hover") ? "bg-[url(/img/n-h-ad-vozidla-1.png)]" : "bg-[url(/img/n-h-ad-vozidla-12.png)]"} ${["tag-DPH", "tag-discount-DPH"].includes(type) ? "overflow-hidden" : ""} ${nHAdVozidlaClassName}`}
        >
        ...
        <div className="w-full flex self-stretch flex-col items-start grow flex-1 pl-2 pr-0 py-0 justify-between relative">
          ...
          <div className="w-full flex self-stretch items-center flex-[0_0_auto] justify-between relative">
            <div className="flex items-end grow gap-2 flex-1 pl-0 pr-2 pt-0 pb-4 h-12 relative">
              <div className="[font-family:'Poppins',Helvetica] w-fit tracking-[0] text-base text-colors-white-1000 font-normal leading-6 whitespace-nowrap relative">
                ...
                <span
                  className={`text-[#a0a0a5] font-semibold ${type === "tag-discount-DPH" ? "text-2xl" : "text-xl"}`}
                >
                  &nbsp;
                </span>
