...
export class ImageProcessor {
  ...
  constructor() {
    ...
    this.apiBaseUrl = import.meta.env.VITE_API_URL || 'https://blackrent-app-production-4d6f.up.railway.app/api';
  }

  /**
   * Spracovanie obrázkov - originál + 800px thumbnail
   */
  async processImages(files: File[], protocolId: string): Promise<ProcessedImage[]> {
   ...
   */
  private async uploadOriginal(file: File, protocolId: string): Promise<string> {
   ...
   */
  private async uploadThumbnail(blob: Blob, protocolId: string, originalFilename: string): Promise<string> {
