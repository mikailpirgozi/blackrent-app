...
export const exportRentalsToCSV = (rentals: Rental[], vehicles: Vehicle[]) => {
  ...
  link.setAttribute('download', `prenajmy-${format(new Date(), 'yyyy-MM-dd')}.csv`);
...
export const applyRentalFilters = (
  ...
  return rentals.filter(rental => {
    ...
    if (filters.paymentMethod?.length > 0 && !filters.paymentMethod.includes(rental.paymentMethod)) {
      return false;
    }

    // Company filter
    if (filters.company?.length > 0 && vehicle && !filters.company.includes(vehicle.company)) {
      return false;
    }
...
export const getUniqueFilterValues = (rentals: Rental[], vehicles: Vehicle[]) => {
  const vehicleMap = new Map(vehicles.map(v => [v.id, v]));
...
export const getProtocolStatusText = (rental: Rental, protocols: Record<string, any>) => {
...
export const getVehicleByRental = (rental: Rental, vehicleMap: Map<string, Vehicle>) => {
