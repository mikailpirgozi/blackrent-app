...
router.get('/full-test', async (req, res) => {
    try {
        ...
        try {
            const queueStatus = await (0, setup_1.checkQueues)();
            results.tests.push({
        ...
        try {
            ...
            // Test upload
            const uploadUrl = await r2_storage_1.r2Storage.uploadFile(testKey, testContent, 'text/plain');
            // Test download
            const downloadedContent = await r2_storage_1.r2Storage.getFile(testKey);
            // Test file exists
            const exists = await r2_storage_1.r2Storage.fileExists(testKey);
            // Cleanup
            await r2_storage_1.r2Storage.deleteFile(testKey);
        ...
        try {
            ...
            const hash = hash_calculator_1.HashCalculator.calculateSHA256(testJpeg);
            results.tests.push({
      ...
      `);
        ...
        try {
            ...
            const routeExists = req.app._router && req.app._router.stack.some((layer) => layer.regexp && layer.regexp.source.includes('v2'));
            results.tests.push({
...
//# sourceMappingURL=v2-system-test.js.map