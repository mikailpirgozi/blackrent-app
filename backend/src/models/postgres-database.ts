...
export class PostgresDatabase {
  ...
  async updateInsurance(id: string, insuranceData: {
    ...
    try {
      const result = await client.query(
        'UPDATE insurances SET type = $1, policy_number = $2, start_date = $3, end_date = $4, premium = $5, coverage_amount = $6, payment_frequency = $7, file_path = $8 WHERE id = $9 RETURNING id, rental_id, insurer_id, policy_number, type, coverage_amount, premium, start_date, end_date, payment_frequency, file_path',
        [insuranceData.type, insuranceData.policyNumber, insuranceData.validFrom, insuranceData.validTo, insuranceData.price, insuranceData.price, insuranceData.paymentFrequency || 'yearly', insuranceData.filePath || null, id]
      );
