...
import { PDFAGenerator, type PDFGenerationRequest } from '../utils/v2/pdf-a-generator';
import { pdfQueue } from './setup';
 ...
 */
pdfQueue.process('build-protocol-pdf', async (job: Job<PDFBuildJob>): Promise<PDFBuildResult> => {
  ...
  try {
    ...
    for (const photo of photos) {
      try {
        ...
        const photoExists = await r2Storage.fileExists(pdfPhotoKey);
        
        if (photoExists) {
          processedPhotos.push({
            ...photo,
            url: await r2Storage.getSignedUrl(pdfPhotoKey, 3600) // 1 hodina
          });
        } else {
          ...
          const galleryExists = await r2Storage.fileExists(galleryPhotoKey);
          
          if (galleryExists) {
            processedPhotos.push({
              ...photo,
              url: await r2Storage.getSignedUrl(galleryPhotoKey, 3600)
            });
          }
        }
      } catch (error) {
 ...
 */
pdfQueue.process('bulk-pdf-build', async (job: Job<{ protocols: PDFBuildJob[] }>): Promise<{
  success: boolean;
  completed: number;
  failed: number;
  results: PDFBuildResult[];
}> => {
  const { protocols } = job.data;
  const results: PDFBuildResult[] = [];
  let completed = 0;
  let failed = 0;
