...
export class RentalRepository extends BaseRepository {
  ...
  private rentalCache = new Map<string, {
    data: Rental[];
    timestamp: number;
  }>();
  private readonly RENTAL_CACHE_TTL = 2 * 60 * 1000; // 2 minúty
  ...
  async getRentals(): Promise<Rental[]> {
    ...
    if (cached && (Date.now() - cached.timestamp) < this.RENTAL_CACHE_TTL) {
      logger.migration('🎯 RENTAL CACHE HIT - returning cached rentals');
      return cached.data;
    }

    logger.migration('🔄 RENTAL CACHE MISS - loading fresh rentals from DB');
   ...
   */
  private async getRentalsFresh(): Promise<Rental[]> {
  ...
  async getRentalsPaginated(params: {
    ...
    startDate?: Date;
    endDate?: Date;
  }): Promise<{ rentals: Rental[]; total: number; page: number; limit: number }> {
    const client = await this.getClient();
    try {
   ...
   */
  private invalidateRentalCache(): void {
    this.rentalCache.clear();
    logger.debug('🗑️ Rental cache invalidated');
  }

  // Ďalšie rental-related metódy budú pridané postupne...
  // getHandoverProtocolsByRental, getReturnProtocolsByRental, etc.
}
