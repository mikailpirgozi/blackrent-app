...
import { pdfQueue, photoQueue } from '../queues/setup';
import { photoServiceV2 } from '../services/photo-service-v2';
  ...
  try {
    ...
    if (photo.status === 'processing') {
      try {
        ...
        const pendingJobs = await photoQueue.getWaiting();
        
        const allJobs = [...activeJobs, ...pendingJobs];
        const photoJob = allJobs.find(job => 
          job.data && job.data.photoId === photoId
        );
...
router.post('/:protocolId/generate-pdf', async (req, res) => {
  try {
    ...
    // Pridanie do queue
    const job = await pdfQueue.add('build-protocol-pdf', {
      protocolId,
      protocolType,
      protocolData,
      userId,
  ...
  try {
    ...
    if (job.status === 'processing') {
      try {
        ...
        const pendingJobs = await pdfQueue.getWaiting();
        
        const allJobs = [...activeJobs, ...pendingJobs];
        const pdfJob = allJobs.find(queueJob => 
          queueJob.data && queueJob.data.protocolId === protocolId
        );
...
router.post('/:protocolId/generate-manifest', async (req, res) => {
  try {
    ...
    // Pridanie do queue
    const job = await pdfQueue.add('generate-manifest', {
      protocolId,
      photoIds,
      userId,
      metadata: {
...
router.delete('/photos/:photoId', async (req, res) => {
  try {
    ...
    const deletePromises = [
      ...
      return r2Storage.deleteFile(key).catch((error: Error) => {
...
router.get('/queue/stats', async (req, res) => {
  try {
    const [photoStats, pdfStats] = await Promise.all([
      {
        waiting: (await photoQueue.getWaiting()).length,
        active: (await photoQueue.getActive()).length,
        completed: (await photoQueue.getCompleted()).length,
        failed: (await photoQueue.getFailed()).length
      },
      {
        waiting: (await pdfQueue.getWaiting()).length,
        active: (await pdfQueue.getActive()).length,
        completed: (await pdfQueue.getCompleted()).length,
        failed: (await pdfQueue.getFailed()).length
      }
    ]);
...
router.post('/migration/rollback/:protocolId', async (req, res) => {
  try {
    ...
    await migrationService.rollbackProtocol(protocolId);
...
router.get('/migration/validate/:protocolId', async (req, res) => {
  try {
    ...
    const validation = await migrationService.validateMigration(protocolId);
