...
export default function NativeCamera({
  ...
  const initCamera = useCallback(
    async (facingMode: 'environment' | 'user' = 'environment') => {
      ...
      try {
        ...
        // 🚀 MAXIMÁLNE ROZLÍŠENIE: Nastavenie constraints pre najvyššiu kvalitu
        const constraints: MediaStreamConstraints = {
          video: {
            facingMode: facingMode,
            width: {
              min: 1280,
              ideal: 2560, // 🚀 ZVÝŠENÉ: z 1920 na 2560px pre ešte vyššiu kvalitu
              max: 4096, // 🚀 ZVÝŠENÉ: z 3840 na 4096px (4K support)
            },
            height: {
              min: 720,
              ideal: 1440, // 🚀 ZVÝŠENÉ: z 1080 na 1440px (QHD)
              max: 2160, // Zachované 4K
            },
            frameRate: { ideal: 30, max: 60 }, // 🚀 NOVÉ: Vyššia frame rate pre plynulosť
            aspectRatio: { ideal: 16 / 9 }, // 🚀 NOVÉ: Preferovaný aspect ratio
          },
          audio: false,
        };
  ...
  const capturePhoto = useCallback(async () => {
    ...
    try {
      ...
      // 🚀 VYSOKÁ KVALITA: Nastavenie canvas s minimálnym rozlíšením
      const minWidth = 1920;
      const minHeight = 1080;

      // Použiť vyššie rozlíšenie z video alebo minimálne Full HD
      const finalWidth = Math.max(video.videoWidth, minWidth);
      const finalHeight = Math.max(video.videoHeight, minHeight);

      canvas.width = finalWidth;
      canvas.height = finalHeight;

      // 🎨 MAXIMÁLNA KVALITA: Najlepšie nastavenia pre smooth scaling
      context.imageSmoothingEnabled = true;
      context.imageSmoothingQuality = 'high';

      // Vykreslenie s upscaling pre maximálnu kvalitu
      context.drawImage(video, 0, 0, finalWidth, finalHeight);

      logger.debug('📸 HIGH QUALITY capture:', {
        originalVideo: `${video.videoWidth}x${video.videoHeight}`,
        finalCanvas: `${finalWidth}x${finalHeight}`,
        upscaled:
          finalWidth > video.videoWidth || finalHeight > video.videoHeight,
      });
      ...
      const captureWithFormat = () => {
        if (webPSupported) {
          ...
          canvas.toBlob(
            ...
            'image/webp',
            0.98 // 🚀 MAXIMÁLNA KVALITA: 98% pre WebP (najlepšia možná kvalita)
          );
        } else {
          // Priamy JPEG capture
          captureJPEG();
      ...
      const captureJPEG = () => {
        canvas.toBlob(
          ...
          'image/jpeg',
          0.98 // 🚀 MAXIMÁLNA KVALITA: 98% pre JPEG (fallback s vysokou kvalitou)
        );
      };
