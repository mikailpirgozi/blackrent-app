...
router.get('/paginated', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'read'), async (req, res) => {
    try {
        const { page = 1, limit = 50, search = '', dateFilter = 'all', dateFrom = '', dateTo = '', company = 'all', status = 'all', protocolStatus = 'all', paymentMethod = 'all', paymentStatus = 'all', vehicleBrand = 'all', priceMin = '', priceMax = '', sortBy = 'smart_priority', sortOrder = 'asc' } = req.query;
...
router.post('/:id/clone', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'create'), // Potrebuje create permission pre nový prenájom
async (req, res) => {
    try {
        // console.log('🔄 RENTAL CLONE ENDPOINT HIT - ID:', req.params.id);
        const { id } = req.params;
        // Získaj originálny prenájom
        const originalRental = await postgres_database_1.postgresDatabase.getRental(id);
        if (!originalRental) {
            return res.status(404).json({
                success: false,
        ...
        const clonedRentalFixed = {
            ...
            approvalStatus: 'pending' // Reset approval status pre nový rental
        };
        // Vytvor nový prenájom v databáze
        const newRental = await postgres_database_1.postgresDatabase.createRental(clonedRentalFixed);
