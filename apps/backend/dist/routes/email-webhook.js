...
router.post('/debug', async (req, res) => {
    try {
        ...
        if (parsedData.customerName) {
            try {
                const customers = await postgresDatabase.getCustomers();
                const customer = customers.find(c => c.name?.toLowerCase() === parsedData.customerName?.toLowerCase() ||
...
router.post('/webhook', async (req, res) => {
    try {
        ...
        if (parsedData.customerName) {
            const customers = await postgresDatabase.getCustomers();
            let customer = customers.find(c => c.name?.toLowerCase() === parsedData.customerName?.toLowerCase() ||
                c.email === parsedData.customerEmail);
        ...
        if (parsedData.handoverDate && parsedData.returnDate) {
            ...
            if (timeMatch) {
                ...
                logger_1.logger.info('✅ Parsed dates from reservationTime (legacy) - PLAIN STRINGS:', {
                    startDate: startDate,
                    endDate: endDate
                });
            }
        ...
        if (!startDate || !endDate || (typeof startDate === 'object' && isNaN(startDate.getTime())) || (typeof endDate === 'object' && isNaN(endDate.getTime()))) {
            ...
            const formatToPlainString = (date) => {
...
router.get('/pending', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'read'), async (req, res) => {
    try {
...
router.post('/approve/:id', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'update'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/reject/:id', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'update'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/rentals/:id', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'update'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (websocketService) {
            try {
                ...
                if (updatedRental) {
                    ...
                    logger_1.logger.info('📢 WebSocket broadcast sent for rental update:', id);
                }
            }
            catch (broadcastError) {
                logger_1.logger.error('❌ WebSocket broadcast failed:', broadcastError);
            }
        }
        res.json({
            success: true,
...
router.get('/stats', auth_1.authenticateToken, (0, permissions_1.checkPermission)('rentals', 'read'), async (req, res) => {
    try {
...
//# sourceMappingURL=email-webhook.js.map